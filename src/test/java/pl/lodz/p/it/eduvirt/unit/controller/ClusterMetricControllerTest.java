...
public class ClusterMetricControllerTest {
    ...
    /* GetClusterMetricDetails method tests */
    ...
    public void Given_ExistingClusterAndMetricIdentifiersArePassed_When_GetClusterMetricDetails_Then_ReturnsFoundClusterMetricDetails() throws Exception {
        ...
        when(clusterMetricService.findClusterMetricByClusterAndMetric(clusterMock, metric1))
                .thenReturn(Optional.of(clusterMetric1));

        when(eTagHelper.generateEtag(clusterMetric1)).thenReturn(eTag);
        
        MvcResult result = mockMvc.perform(get("/clusters/{clusterId}/metrics/{metricId}", 
                        existingClusterId, metric1.getId()))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(header().exists(HttpHeaders.ETAG))
                .andReturn();

        String attachedETag = result.getResponse().getHeader(HttpHeaders.ETAG);
        assertNotNull(attachedETag);
        assertEquals("\"%s\"".formatted(eTag), attachedETag);

        String json = result.getResponse().getContentAsString();
        GeneralMetricValueDto outputDto = mapper.readValue(json, GeneralMetricValueDto.class);

        assertNotNull(outputDto);
        assertEquals(outputDto.id(), clusterMetric1.getId());
        assertEquals(outputDto.version(), clusterMetric1.getVersion());
        assertEquals(outputDto.value(), clusterMetric1.getValue());
        assertEquals(outputDto.metric().id(), metric1.getId());
        assertEquals(outputDto.metric().name(), metric1.getName());
        assertEquals(outputDto.metric().category(), metric1.getCategory());

        verify(clusterService, times(1)).findClusterById(existingClusterId);
        verify(metricService, times(1)).findById(metric1.getId());

        verify(clusterMetricService, times(1))
                .findClusterMetricByClusterAndMetric(clusterMock, metric1);

        verify(eTagHelper, times(1)).generateEtag(clusterMetric1);
    }
    ...
    public void Given_NonExistentClusterIdentifierIsPassed_When_GetClusterMetricDetails_Then_Returns404NotFound() throws Exception {
        ...
        mockMvc.perform(get("/clusters/{clusterId}/metrics/{metricId}",
                        nonExistentClusterId, metric1.getId()))
                .andDo(print())
                .andExpect(status().isNotFound());
    ...
    public void Given_NonExistentMetricIdentifierIsPassed_When_GetClusterMetricDetails_Then_Returns404NotFound() throws Exception {
        ...
        when(clusterService.findClusterById(existingClusterId)).thenReturn(clusterMock);
        when(metricService.findById(nonExistentMetricId)).thenThrow(MetricNotFoundException.class);

        mockMvc.perform(get("/clusters/{clusterId}/metrics/{metricId}",
                        existingClusterId, nonExistentMetricId))
                .andDo(print())
                .andExpect(status().isNotFound());

        verify(clusterService, times(1)).findClusterById(existingClusterId);
        verify(metricService, times(1)).findById(nonExistentMetricId);
    }
